...
class AddBhojanshalaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val etOpeningTime = findViewById<EditText>(R.id.etOpeningTime)
        val etClosingTime = findViewById<EditText>(R.id.etClosingTime)
        ...
        btnSubmit.setOnClickListener {
            ...
            val openingTime = etOpeningTime.text.toString().trim()
            val closingTime = etClosingTime.text.toString().trim()
            ...
            if (token != null && name.isNotEmpty() && city.isNotEmpty() && address.isNotEmpty() && openingTime.isNotEmpty() && closingTime.isNotEmpty()) {
                val file = selectedImageUri?.let { getFileFromUri(it) }
                viewModel.submitNewBhojanshala(token, name, city, address, openingTime, closingTime, contact, description, file)
            } else {
                Toast.makeText(this, "Please fill required fields (Name, City, Address, Timings)", Toast.LENGTH_SHORT).show()
            }
        }
